...
const SalesModal = ({ formData, setModalLoading, modalLoading, data, open, onClose, children }) => {
    ...
    useEffect(() => {
        if (data?.salesPersonAllOrders) {
            const createRows = salesModalRows(data?.salesPersonAllOrders)
            const createColumn = createColumnForSalesModal(data?.salesPersonAllOrders)
            setRows(createRows)
            ...
            >
                ...
                >
                    ...
                    <Box sx={{ fontWeight: "bold", fontSize: "18px" }}>
                        {data?.salesPersonName?.toUpperCase() || ''}
                        ({`Sales Amount: ₹ ${new Intl.NumberFormat('en-IN').format(data?.totalAmount)}  `})
